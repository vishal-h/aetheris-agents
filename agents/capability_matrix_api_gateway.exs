# agents/capability_matrix_api_gateway.exs
#
# Capability matrix sub-agent: api/gateway side.
# Reads agents and scripts under api/gateway/ and writes docs/.sections/api_gateway.md.
#
# Run as part of the capability_matrix sprint case, or standalone:
#   mix aetheris run ../aetheris-agents/agents/capability_matrix_api_gateway.exs
...
%Aetheris.RunConfig{
  ...
  write a capability section in Markdown. Gateway-side agents: cot1 (TAP gateway). Connects to ct.stu API.

  Workflow — follow these steps in order:

  Step 1: List agents.
    Call list_dir with path: "api/gateway/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "api/gateway/agents/{filename}"
    Extract:
  ...
  Step 3: List scripts.
    Call list_dir with path: "api/gateway/scripts"
    Collect .py and .exs filenames. Skip __init__.py and conftest.py.

  Step 4: For each .py or .exs script:
    Call read_file with path: "api/gateway/scripts/{filename}"
  ...
  Step 5: Write the section file.
    Call write_file with:
      path: "docs/.sections/api_gateway.md"
