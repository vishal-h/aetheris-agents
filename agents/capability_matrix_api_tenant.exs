# agents/capability_matrix_api_tenant.exs
#
# Capability matrix sub-agent: api/tenant side.
# Reads agents and scripts under api/tenant/ and writes docs/.sections/api_tenant.md.
#
# Run as part of the capability_matrix sprint case, or standalone:
#   mix aetheris run ../aetheris-agents/agents/capability_matrix_api_tenant.exs
...
%Aetheris.RunConfig{
  run_id:           "cap-matrix-api-tenant-#{Aetheris.ID.generate()}",
  mode:             :record,
  provider:         provider,
  model:            model,
  ...
  write a capability section in Markdown. Tenant-side agents: at1cmd (dispatcher), at1qry (collector).

  Workflow — follow these steps in order:

  Step 1: List agents.
    Call list_dir with path: "api/tenant/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "api/tenant/agents/{filename}"
    Extract:
  ...
  Step 3: List scripts.
    Call list_dir with path: "api/tenant/scripts"
    Collect .py and .exs filenames. Skip __init__.py and conftest.py.

  Step 4: For each .py or .exs script:
    Call read_file with path: "api/tenant/scripts/{filename}"
  ...
  Step 5: Write the section file.
    Call write_file with:
      path: "docs/.sections/api_tenant.md"
