# agents/capability_matrix_cloudcost.exs
#
# Capability matrix sub-agent: cloudcost use case.
# Reads agents and scripts under cloudcost/ and writes docs/.sections/cloudcost.md.
#
...
%Aetheris.RunConfig{
  run_id:           "cap-matrix-cloudcost-#{Aetheris.ID.generate()}",
  mode:             :record,
  provider:         provider,
  model:            model,
  label:            "cap-matrix: cloudcost",
  ...
  Step 1: List agents.
    Call list_dir with path: "cloudcost/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "cloudcost/agents/{filename}"
    Extract:
      - The label: "..." value (agent's human name)
      - The tools: [...] list

  Step 3: List scripts.
    Call list_dir with path: "cloudcost/scripts"
    Collect ALL .py filenames, INCLUDING underscore-prefixed shared helpers like
    `_normalized.py` — those are shared utilities, not private; they belong in the
    table. Skip only __init__.py and conftest.py.

  Step 4: For each .py script:
    Call read_file with path: "cloudcost/scripts/{filename}"
  ...
  Step 5: Write the section file.
    Call write_file with:
      path: "docs/.sections/cloudcost.md"
  ...
  """,
  user_prompt: "Generate the capability section for the cloudcost use case. Begin."
}
