# agents/capability_matrix_drive.exs
#
# Capability matrix sub-agent: drive use case.
# Reads agents and scripts under drive/ and writes docs/.sections/drive.md.
#
# Run as part of the capability_matrix sprint case, or standalone:
#   mix aetheris run ../aetheris-agents/agents/capability_matrix_drive.exs
...
%Aetheris.RunConfig{
  run_id:           "cap-matrix-drive-#{Aetheris.ID.generate()}",
  mode:             :record,
  provider:         provider,
  model:            model,
  ...
  Step 1: List agents.
    Call list_dir with path: "drive/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "drive/agents/{filename}"
    Extract:
