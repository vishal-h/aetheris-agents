# agents/capability_matrix_email.exs
#
# Capability matrix sub-agent: email use case.
# Reads agents and scripts under email/ and writes docs/.sections/email.md.
#
# Run as part of the capability_matrix sprint case, or standalone:
#   mix aetheris run ../aetheris-agents/agents/capability_matrix_email.exs

agent_root = Path.expand(Path.join(Path.dirname(__ENV__.file), ".."))

%Aetheris.RunConfig{
  run_id:           "cap-matrix-email-#{Aetheris.ID.generate()}",
  mode:             :record,
  provider:         "anthropic",
  ...
  Step 1: List agents.
    Call list_dir with path: "email/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "email/agents/{filename}"
    Extract:
  ...
  Step 3: List scripts.
    Call list_dir with path: "email/scripts"
    Collect .py filenames only. Skip __init__.py and conftest.py.

  Step 4: For each .py script:
    Call read_file with path: "email/scripts/{filename}"
