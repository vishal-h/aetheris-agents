# agents/capability_matrix_provenance.exs
#
# Capability matrix sub-agent: provenance use case.
# Reads agents and scripts under provenance/ and writes docs/.sections/provenance.md.
#
# Run as part of the capability_matrix sprint case, or standalone:
#   mix aetheris run ../aetheris-agents/agents/capability_matrix_provenance.exs
...
%Aetheris.RunConfig{
  ...
  Step 1: List agents.
    Call list_dir with path: "provenance/agents"
    Collect .exs filenames only. Ignore non-.exs files.

  Step 2: For each .exs agent file:
    Call read_file with path: "provenance/agents/{filename}"
    Extract:
  ...
  Step 3: List scripts.
    Call list_dir with path: "provenance/scripts"
    Collect .py filenames only. Skip __init__.py and conftest.py.

  Step 4: For each .py script:
    Call read_file with path: "provenance/scripts/{filename}"
  ...
  Step 5: Write the section file.
    Call write_file with:
      path: "docs/.sections/provenance.md"
  ...
  """,
  user_prompt: "Generate the capability section for the provenance use case. Begin."
}
