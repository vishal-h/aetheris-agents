...
Base address `ai@bitloka.com`; intent via plus-addressing: `ai+invoice@`,
...
| Email | `From:` **AND** `Authentication-Results` showing spf/dkim/dmarc pass for `bitloka.com` | The From header alone is forgeable — the DMARC conjunct is what makes the allowlist real rather than theatre. |
