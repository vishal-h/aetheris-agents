...
`[Noted 2026-09-17 at the BL-262 close. The 2026-09-10 block above says four callers bind `%{RunConfig.from_map(...) | ...}` and cannot consume an `{:error, _}`. That is no longer true. From harness `b079130`, `RunConfig.from_map/3` returns `{:ok, config} | {:error, reason}`, and every caller in `lib/` consumes both arms (`grep -rn 'RunConfig.from_map' lib/`). BL-045's total-decode amendment therefore no longer needs to change those callers' shape; what it still needs is a new error reason. Not re-ruled here.]`
