...
> **[Closed 2026-09-15: `state: done`, disposition `fixed`, by `c0f9f1b` (BL-253's procedure edit).]**
> - **Scope stated by rule, not pointer.** Step 5 now opens its remove half with **REMOVE — scope:
>   the manifest set, and nothing else.** The rule was chosen on this row's own ground: the reader
>   is a human making an irreversible deletion in a UI.
> - **`claude/` is legible at the deletion.** The same paragraph says **Remove nothing under
>   `claude/`**, and says why.
> - **The history question, answered.** The six-kind enumeration was written with the prompt at
>   `815cf21` (2026-06-11), when the manifest had twelve rows. At least four of them fell under no
>   named kind: `agent-creation-guide.md`, `capability-matrix.md`, `backlog-2026-06.md` and
>   `project-knowledge-manifest.md`. The list was therefore incomplete from the day it was written.
>   `d7d7476` (2026-09-08) replaced it with *"everything at a bare export name"*. Source:
>   `git log -S 'stale handoff' -- prompts/bl-002-refresh-project-knowledge.md`.
> - **The 2026-08-19 appended finding** is carried by the same edit's route rule: on the store
>   route, replace in place and never remove a name about to be replaced.
> - **Citation moved.** The 2026-08-18 boundary record cited above now lives in
>   `docs/export-boundaries/2026-08-18.md`.
