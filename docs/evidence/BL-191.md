...
> **[2026-09-15, partial: clauses 1, 3 and 4 done agents-side; clause 2 STOPPED, needs a harness write. Row stays open.]**
> - **Clause 1.** `drive/runbook.md` now names `DRIVE_ROOT_FOLDER_ID` throughout, with the `YYYY-MM/` period-folder model the scripts use (`drive_upload.py:175-186`, `drive_download.py:110`). The two dead troubleshooting entries are merged into `### DRIVE_ROOT_FOLDER_ID not set`, which quotes the message both scripts emit. Swept with it:
>   - `drive/README.md`, the env table.
>   - `email/README.md` and `email/runbook.md`: the template folder is `DRIVE_TEMPLATES_FOLDER_ID`, the variable `email_download_template.py:75` reads. The per-month folder-ID ritual is replaced. A `DEBUG: folder_id=` print that no script emits is removed. The template-miss entry now quotes the real message.
>   - `docs/agent-creation-guide.md`, the Shared Drive paragraph.
>
>   Three quoted messages the scripts cannot emit were corrected in the same lines: the credential message, the download success block, and the template miss.
> - **Clause 3.** The upload Expected-output block is now `N uploaded, 0 failed.`, established by running `drive_upload.py`'s `main()`. The only stub was the Drive service, so this is a stated substitution for a live upload, which would write to Drive. It printed `4 uploaded, 0 failed.` and nothing else on stdout. The download block was checked the same way (`Downloaded: payroll.csv (modified …)` / `Saved to: …`). Every missing-variable message was taken from a real run.
> - **Clause 4, the sweep at the fixing commit.** Reproduce it with `git grep -c <VAR>` in each repo, for `DRIVE_OUTPUT_FOLDER_ID` and `DRIVE_PAYROLL_FOLDER_ID`. No count is recorded here, because this block names the variables itself. What still names a dead variable, per file:
>   - Harness `scripts/sprint.sh`: clause 2, below.
>   - Records, deliberately not swept:
>     - This file and the row in `docs/backlog-2026-06.md`.
>     - `docs/reviews/bug-001-review.md` and `docs/reviews/bug-001-review-r1.md`.
>     - `drive/docs/bug-001-implementation-notes.md`.
>     - `docs/rig/milestones/p8/p8-002-drive-folder-convention.md`, the rename ticket.
>     - `drive/milestone.md` and `email/milestone.md`, the dated t-ticket specs, treated as records on the same ground as `p8-002`.
>
>   No standing agents surface names either dead variable.
> - **Clause 2, not done.** `../aetheris/scripts/sprint.sh:1061` still gates the drive arm on `DRIVE_PAYROLL_FOLDER_ID DRIVE_OUTPUT_FOLDER_ID`, and `:1137` still passes `DRIVE_OUTPUT_FOLDER_ID` to `email_download_template.py`. Both are harness lines, outside the agents-only session that did the rest. Until they change, `drive/runbook.md`'s `./scripts/sprint.sh drive` instruction points at an arm whose own prerequisite loop fails.

> **[Closed 2026-09-15: `state: done`, disposition `fixed`.]** Clauses 1, 3 and 4 landed agents-side at `7a94164`, recorded in the partial block above. Clause 2 landed at harness `06179d9`.
> - **Clause 2.** All in `../aetheris/scripts/sprint.sh` at `06179d9`:
>   - **The drive arm's prerequisite gate.** It resolves the key path the way `drive_download.build_service` reads it: `GOOGLE_SERVICE_ACCOUNT_FILE`, falling back to `GOOGLE_SERVICE_ACCOUNT` (`SA_KEY=` at `:1072`). It then requires `DRIVE_ROOT_FOLDER_ID` and `PAYSLIP_MONTH` (the loop at `:1079`). `drive_upload.py` and `email_download_template.py` import that same `build_service`, so one key pair covers all three scripts.
>   - **The key-file `-f` check.** It now reads the same resolved path. This is one line beyond the ticket: left on `${GOOGLE_SERVICE_ACCOUNT}`, it would die on an unbound variable under `set -u` when only `GOOGLE_SERVICE_ACCOUNT_FILE` is set.
>   - **The `Requires:` header** (`:1053`) names the same set.
>   - **The email arm's template download** (`:1155`) passes `DRIVE_TEMPLATES_FOLDER_ID`, the variable `email_download_template.py:75` reads.
> - **Verified by dry construction, not by a sprint run.** The arms spend LLM and Drive calls. Their own text ran under controlled environments, with nothing reaching Drive.
>   - Gate at `d34bc9f`: with the scripts' variables set, it hits `exit 1` on `GOOGLE_SERVICE_ACCOUNT is not set`. With only the dead pair set, it passes. This is the defect.
>   - Gate at `06179d9`: three configurations reach the orchestrator — the scripts' variables, the legacy key alone, and an empty `GOOGLE_SERVICE_ACCOUNT_FILE` beside a set legacy key. Four stop with `exit 1`, each at its own message: only the dead pair, no month, no key, and a missing key file.
>   - Email pass, with `python3` stubbed to report what it received. An exported `DRIVE_TEMPLATES_FOLDER_ID` reached the script under both texts, through inheritance. A variable set but not exported reached it only under `06179d9`.
> - **Clause 4, re-swept at the fixing commit.** `git -C ../aetheris grep -c 'DRIVE_OUTPUT_FOLDER_ID\|DRIVE_PAYROLL_FOLDER_ID' 06179d9` returns nothing; the same pattern found 3 lines in `scripts/sprint.sh` at `d34bc9f`. Agents-side, the grep names the same files it named at `7a94164`, all of them records listed above. The one change is that this row's heading now sits in `docs/backlog-2026-06-closed.md`.
> - **Still not established: whether the drive arm ran between `72367ac` and this fix.** Sprint output goes to the gitignored `sprint/<timestamp>/`. This machine's 63 run directories hold no `drive/`, but they hold no `payslip/` or `email/` either, so the absence shows nothing.
