...
> - **Rejection before work.** Directly after the `if not args.month` presence guard, and before `load_config`, `email/scripts/email_send.py` requires `re.fullmatch(r"\d{4}-(0[1-9]|1[0-2])", …)`. Otherwise it prints `--month/PAYSLIP_MONTH must be YYYY-MM, got: <repr>` and calls `sys.exit(1)`. The display `strptime` stays and only formats a validated value. The check is stricter than `strptime("%Y-%m")`, which accepts `2026-4`. Run directly, `--month '2026-*'` and `PAYSLIP_MONTH=2026-4` each print the message and exit 1.
> - **Test.** `test_main_rejects_malformed_month_before_any_work` covers `2026-*`, `2026-4`, `2026-13`, `April 2026` and `2026-04-01`, asserting the message, exit 1, and that neither `load_config` nor `get_employees` was called. `test_main_rejects_malformed_payslip_month_env` covers the same rejection via the env var. With the check removed, all six cases fail. Restored from a copy verified by sha, they pass.
> - **Not changed:** `drive/scripts/drive_upload.py`, per the Not-done-when.
