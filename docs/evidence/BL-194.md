...
> **[2026-09-15, partial: mover and dry-run report done; the move is STOPPED pending review of the report. Row stays open.]**
> - **The mover.** `drive/scripts/drive_quarantine.py`, tested by `drive/tests/test_drive_quarantine.py` against an in-memory Drive with no `delete`. It works in two invocations:
>   - `--period YYYY-MM --report FILE` holds only `drive.readonly`, calls no write, and refuses to overwrite a report.
>   - `--execute --report FILE` re-checks each reported object, moves it with `files().update(addParents, removeParents)` into `payslip-quarantine/<period>/<employee>/` under the payslips root's own parent (outside `payslips/`), re-runs the identity test, and writes `FILE.executed.json`. It refuses to run a report twice.
>
>   An object with no month prefix is reported as unclassifiable and never moved.
> - **The dry run, live, read-only, 2026-09-15T07:08:27Z.** `python3 drive/scripts/drive_quarantine.py --period 2026-07 --report drive/output/bl-194/2026-07-dry-run.json` exited 0 with `{"objects": 73, "clean": 36, "misplaced": 36, "unclassifiable": 1}`.
>   - The 36 misplaced objects are all `2026-05`, two per employee folder, across all 18 folders.
>   - The unclassifiable object is `2026-07/payroll.csv`, at the period folder's top level, which the filing read did not count.
>   - The quarantine parent is not the payslips root.
> - **The report, archived rather than committed** (it carries Drive ids): `drive/output/bl-194/2026-07-dry-run.json`, gitignored by `drive/.gitignore:1`, 12,311 bytes, sha256 `bf991d42493f1c788d0c67771d0f0b8b014b08348d0e3e10d1542522ee68d473`. It names every pair. Those pairs, stated here without ids: for each employee folder `E` in `BTL_01, BTL_010, BTL_014, BTL_015, BTL_016, BTL_017, BTL_018, BTL_02, BTL_020, BTL_026, BTL_028, BTL_029, BTL_03, BTL_030, BTL_035, BTL_036, BTL_06, BTL_Consult_01`, the two moves are `2026-07/E/2026-05-Payslip.csv → payslip-quarantine/2026-07/E/2026-05-Payslip.csv` and `2026-07/E/2026-05-Payslip.pdf → payslip-quarantine/2026-07/E/2026-05-Payslip.pdf`. `2026-05/` and `2026-08/` were not read.
> - **What closes the row.** After the report is reviewed, and after confirming its sha still matches, run `python3 drive/scripts/drive_quarantine.py --execute --report drive/output/bl-194/2026-07-dry-run.json` under a credential with the `drive` scope. The row closes when `drive/output/bl-194/2026-07-dry-run.json.executed.json` shows 36 moved, 0 skipped and `counts_after.misplaced` 0.

> **[Closed 2026-09-15: `state: done`, disposition `fixed`.]** Checked against the done-when. The move ran on the arbiter's instruction, after the report was reviewed.
> - **Report unchanged since review.** Before execution, `drive/output/bl-194/2026-07-dry-run.json` still hashed to sha256 `bf991d42493f1c788d0c67771d0f0b8b014b08348d0e3e10d1542522ee68d473`, and no execution record existed.
> - **Executed 2026-09-15T10:02:20Z.** `python3 drive/scripts/drive_quarantine.py --execute --report drive/output/bl-194/2026-07-dry-run.json` exited 0 with `{"status": "ok", "mode": "executed", "period": "2026-07", "moved": 36, "skipped": 0, "misplaced_after": 0}`. The record is `drive/output/bl-194/2026-07-dry-run.json.executed.json`, archived and gitignored beside the report, sha256 `82167c969f5dd47c89a35460154f961d3207983d7970742158461b9d27b94866`.
> - **`2026-07/` passes the identity test, confirmed independently of the script's own re-scan.** A fresh read-only dry run, `--period 2026-07 --report drive/output/bl-194/2026-07-post-execute-check.json`, reported 37 objects: 36 clean, 0 misplaced, and 1 unclassifiable (`2026-07/payroll.csv`, never moved).
> - **The 36 objects are in a quarantine outside `payslips/`.** A read-only listing found `payslip-quarantine/` under the payslips root's parent, not under the root: the root has no child of that name. `payslip-quarantine/2026-07/` holds 18 employee folders with 2 objects each, 18 `2026-05-Payslip.pdf` and 18 `2026-05-Payslip.csv`. They were moved, not deleted.
> - **Reversible from the report.** Each pair in the report records the object's `file_id` and `source_parent_id`. Moving an object back is `files().update(fileId, addParents=<source_parent_id>, removeParents=<its quarantine folder>)`.
> - **Not touched:** `2026-05/` and `2026-08/`. The mover lists only the period folder it is given.
