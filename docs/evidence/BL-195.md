...
> **[Closed 2026-09-15: `state: done`, disposition `fixed`.]** Checked against the done-when, in the closing commit:
> - **One file.** `email/agents/` holds only `email_orchestrator.exs`. `email_orchestrator2.exs` is deleted. The survivor keeps the name every wiring site already uses, so `OrchestratorView.tsx:18`, `../aetheris/scripts/sprint.sh:1150` and `orchestrator-agent-spec.md:117` name it without being edited. That also means the harness is untouched. Both callers set `PAYSLIP_MONTH` before the file is evaluated (`sprint.sh:1147`, `agents/orchestrator.exs:293`).
> - **Content.** orch1's prompt with its `timeout_ms` hints, plus orch2's eval-time `raise` (`:2`) and an explicit `--month` on step 2. Agent-eval from `../aetheris`: without `PAYSLIP_MONTH` → `** (RuntimeError) PAYSLIP_MONTH not set`, exit 1; with `PAYSLIP_MONTH=2026-08` → evaluates, exit 0.
> - **Matrix.** Regenerated by run `cap-matrix-email-Y8cv7A` plus `scripts/assemble_matrix.py`, not hand-edited. The Email section lists one agent, and the Summary total moved 27 → 26.
> - **Test.** `email/tests/test_email_orchestrator.py` pins the four properties above. Each test fails under its own mutation.
