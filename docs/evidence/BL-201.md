...
> **[Appended 2026-09-15, BL-253.]** The split this row said nothing addressed has landed:
> BL-253 moved the manifest's boundary records and inclusion rulings to `docs/export-boundaries/`.
> In the bundle assembled at agents `ea915cf` / harness `83a79c5` the manifest is 18,975 bytes and
> the ten kernel documents total 243,025, still about twice the 120 KB budget.

> **[Closed 2026-09-15: `state: done`, disposition `fixed`.]** Against the done-when, at agents `527f1d1`:
> - **Arm.** `check_kernel_budget`, check 13 of `scripts/drift_check.py`, sums `_manifest.git_show`
>   over `_manifest.export_rows()` in each row's owning repo. It WARNs, not strict-exempt, above the
>   ceiling, printing the sum, the ceiling and the three largest rows, and FAILs when the header
>   figures do not parse. Green on landing: `kernel 242,533 bytes over 10 rows ≤ ceiling 250,000;
>   119,653 bytes above the 122,880-byte design target`.
> - **Red fixture.** `test_kernel_budget_ceiling_below_the_sum_is_warn_and_not_strict_exempt`
>   (ceiling 499, fixture sum 500). Moving the ceiling to 501 clears the WARN and fails the test.
> - **Marker.** Removed from the manifest header in `527f1d1`.
> - **Cap.** The header carries two figures: a design target of 120 KB (122,880 bytes), not
>   enforced, and a decrease-only ceiling of 250,000 bytes, enforced. This departs from the
>   Not-done-when above (*"raising the cap so the arm is green on landing"*) on the arbiter's
>   ruling of 2026-09-15, given in that round's cc:prompt and recorded here: the split did not
>   close the gap, so the enforced bar is a ratchet at the measured size and 120 KB stays the target.
