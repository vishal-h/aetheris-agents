...
> **[2026-09-15, BL-193.]** The orchestrator's outer cap that arm 1's budget is measured against above (*"The cap fires at `run_start + 300 s`"*) no longer exists. BL-193 removed the `Task.yield(task, 300_000)` wait from `agents/orchestrator.exs`. Arm 1's "step timeout" is now the step's own `timeout_ms: 300000` (`drive/agents/drive_upload_orchestrator.exs:23`), counted from `tool_called`, so the script's share is 300 s rather than ≈298.3 s. The harness's `await_run` inactivity bound is also 300 000 ms and resets on activity. The rebuild's 159.701 s is 53.2% of that budget.
