...
**Status:** **CLOSED 2026-08-09** — hc-a through hc-e all closed; **hc-e closed at r9**, ten rounds,
on Done-check step 7. **The round closes with seven `[OPEN]` §Not established items, one
`[DECIDED]`, and the export boundary verified** — the seven and the one derived from the section's
own prefixes at `f909b95` over a population of twelve, the other four `[RESOLVED]`. The boundary was
verified from **both** sides: the project read back at 28 documents (25 export rows set-compared
both directions, plus three untracked `claude/` briefs) with four named content checks landing, and
`drift_check --strict` at **zero `project_knowledge` WARNs** — the truth-maker, and necessary
without being sufficient. The open items are **recorded, not resolved**, which is **R21** working
rather than the close failing. **Opened:** 2026-08-08.
...
`2026-08-23, appended at the BL-176/BL-179/BL-177 batch — R32's boundary, stated because R32 has been applied outside it in each of the last two tickets. R32 covers a record that was WRONG WHEN WRITTEN. Two neighbours resemble it and are governed elsewhere. FIRST, a statement TRUE when written that later became false is supersession, and the harness CLAUDE.md already rules on it in section Continuous learning, subsection Workflow patterns, in three places, cited here by their headline sentences so that a moved line cannot break the citation: "A document that quotes repo state is a snapshot with no invalidation — verify before acting, and when they disagree the source wins", whose four sources are each described there as true when written, and which says to correct the document in the same pass rather than work around it; "A gate claim is corrected by repointing at what discharged it, and sequencing belongs in one artifact", for the sub-case where the staleness is a closure, and which says to repoint the reader at what the closed work produced rather than delete the pointer; and, for the METHOD, "An artifact's kind decides how a correction is made; its push state decides only whether the correction may be silent", which licenses a silent in-place correction of an UNPUSHED record and requires a dated one once the record has readers, and a dated block quoting the superseded wording in the position it occupied for a ratified decision. Those three were read at harness 9b76009; read them rather than this line, which points and does not restate. SECOND, the choice between amending a commit and appending another is neither R32's question nor supersession's: it is the agents CLAUDE.md rule "Amend a commit while it is private to the ticket and uncited; append once a packet has been issued against it", under section Definition of done, and it keys on whether a packet has cited the commit, not on whether anything in it was wrong. R32 itself is unchanged and unnarrowed by this — it says what to do with a record wrong from the start, and that is still what it says. Landed under R22.`

`2026-09-21, superseded in part at BL-281 — the quotation in SECOND is no longer the rule's text. BL-280 (2026-09-20) re-keyed the agents CLAUDE.md amend rule, under section Definition of done, from citation to push state, and kept the packet as a separate limb: once a packet has been issued against a commit, it is amended only on the arbiter's instruction. The superseded sentence, "Amend a commit while it is private to the ticket and uncited; append once a packet has been issued against it", stands above as it was quoted when true; read the rule rather than the quotation. SECOND's point, that amend-versus-append is neither R32's question nor supersession's, is unchanged. Evidence: docs/evidence/BL-280.md.`
...
`2026-08-23, appended at the BL-176/BL-179/BL-177 batch — where a known member of the class already exists, IT is the control, and a plant is the second reading rather than the first. A plant proves the instrument finds the shape its author imagined; the author of the instrument and the author of the plant are the same person, so the plant inherits every blind spot the instrument has and cannot report one. A known member does not: it was found by someone else, for another reason, before this instrument existed. The instance is BL-181's census. Its R34 plant PASSED — against v1, and again against v2, both of which were under-reporting at the time; the under-reporting was caught only by a second control aimed at BL-135's known site, and that ticket's own notes record the consequence in one line, that the plant alone would have certified a broken instrument. So R34's remedy has an order where the class has a prior member: known member first, plant second, plant alone only where there is no prior member to use. Landed under R22.`
...
| **hc-e** | The close: §7's ritual including its prior-claims census over m4's seven promoted entries, the export boundary, the milestone summary | **Closed** 2026-08-09 at **r9**. The export boundary's Done-check step 7 ran and **both halves passed**. **The project side:** the store was read back through Chrome against the operator-supplied URL and reports **28 documents** — the **25** manifest export rows, set-compared **both directions** with no member on either side, plus exactly the three `claude/` session briefs (`aetheris--playground-token-config-defect`, `aetheris-agents--inbox-brief`, `aetheris-agents--ravenmigrate-brief`), which are not manifest-tracked and were correctly untouched; the manifest document *in the project* carries the **2026-08-09** export-boundary block with all four re-pinned hashes (`dcf1d42`, `7dbdb7d`, `2ef0517`, `2ebc59c`) beside the four they replaced; and **four content checks**, one per re-pinned row, **named before they were run** and each derived from what this round changed, landed in the document that should carry each — the packet-publishes-the-invocation rule in `aetheris-agents--CLAUDE.md`, the positive-control rule and its census provenance in `aetheris--CLAUDE.md`, BL-135's fold onto BL-075 in `backlog-2026-06.md`, and hc-d's sprint exit contract in `aetheris--runbook.md`. **The repo side:** `drift_check.py --strict`, bound by absolute path, returned **9 PASS 0 FAIL 0 WARN, exit 0** — **zero `project_knowledge` WARNs**, the boundary's own truth-maker, and **necessary but not sufficient**: check 8 compares the manifest against git and is structurally blind to a partial or misnamed upload, which is the direction the project read-back exists to cover. **Ten rounds** (r0–r9; the two `hc-e anatomy` commits that fall inside the range are reviewer-authored section-scoped edits and are **not** rounds, per **R20**). Ranges derived from the repos by subject-scoped `git log`, not from any packet: agents **from** `9fbba09` — the range closes at this commit, whose hash a row cannot name from inside itself, which is hc-c's shape and for hc-c's reason; harness `02db6bb`→`2ef0517` (three commits). **Earlier states, kept per decision 7 — the r8 cell entire, including the states it had already nested:** *“**Reopened and stopped twice** 2026-08-09, at agents `3d79a6f`, against the anatomy authored at `e047dd1` and corrected at its r1. The held push was released and C1/C2 landed; the step-1 gate then ran **G1, G2, G3 → proceed** (§7's two steps quoted from the methodology; m4's seven located at HEAD with the four/three split derived; six candidates enumerated with controls) and **stopped at G4**: five of the eight `[OPEN]` §Not established items name no resolver, and §Not carried asserts each does. **No close work** — §7's ritual, the export boundary and the milestone summary are untouched, and G5/G6 were not run. That stop was upheld and adjudicated: **R21** landed, the five were re-labelled, §Not carried was corrected and **G4 was amended** — the amendment tested against both trees and shown stricter, not weaker. **G4 re-run → proceed; the ticket then stopped again at G5**, on §Close criteria clause 3, whose three-value disposition set could not express the `[DECIDED]` prefix R21 introduced. Both stops upheld: clause 3 corrected under authorisation, the vocabulary swept for other consumers (three corrected, one refuted, one reported ambiguous), item 5's settling condition sharpened, and a candidate filed. **G5 and G6 re-run → proceed.** §7's **prior-claims census** ran over all three promotion-claim blocks, every item, and found one rule absent in substance — *a negative needs a positive control* — now promoted to `../aetheris/CLAUDE.md` and verified by reading it back. **Stopped at a stated point inside §7**, before the distillation of this round's seven candidates and before the Done-check. **Still no close work; the export boundary was not approached.** **Earlier states, kept per decision 7:** *"**Reopened** 2026-08-09 … **In progress** — its step-1 gate G1–G6 has not yet returned, so no outcome is claimed here."*, and before that *"**Opened and stopped** 2026-08-09 at agents `9fbba09`. Its opening edit landed — hc-d's row closed under R19, the recency-selection candidate filed, BL-135 established as a duplicate of BL-075 and folded, and BL-075's arm-2 blocker established as partly lifted (the durable place exists; the routing does not). The ticket then stopped at its anatomy census: **2 of 7 §6 fields authored and no step-1 gate slot at all**. No close work. Anatomy and gate authored by the reviewer's section-scoped edit of 2026-08-09 (this commit); hc-e reopens against it."*, and before that *"Not started"*”* |
