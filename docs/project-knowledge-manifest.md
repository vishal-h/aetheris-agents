...
- **Kernel ceiling: 236,000 bytes**, the enforced bar. At each export boundary, with the measured
  ...
  requires a separate recorded inclusion ruling in `export-boundaries/rulings.md`. A new kernel row
  still names what it displaces, or it triggers a probe re-run (design §3) to justify the growth.
  The 10,000 is the margin at the boundary that sets the ceiling, not a standing guarantee: a
...
- THE SYNTHESIS DEMOTION (2026-09-21) — `aetheris--bl-008-synthesis.md` goes from `both` to `on-demand`, executing the transition ratified D-B deferred ("demote after"); BL-202 requires no new probe · [rulings.md](export-boundaries/rulings.md) — "THE SYNTHESIS DEMOTION — `aetheris--bl-008-synthesis.md` goes from `both` to `on-demand`,"
- THE CEILING RATCHET MOVES TO THE BOUNDARY (2026-09-21) — at each boundary `new_ceiling = min(current_ceiling, ceil_1000(measured + 10,000))`; the margin holds only where the ceiling is set · [rulings.md](export-boundaries/rulings.md) — "THE CEILING RATCHET MOVES TO THE BOUNDARY, 2026-09-21 (R3). At each export boundary the"
- THE BACKLOG ROW IS D-B'S QUEUE (2026-09-21) — `backlog-2026-06.md` is D-B's post-split queue and stays `export` · [rulings.md](export-boundaries/rulings.md) — "THE BACKLOG ROW IS D-B'S QUEUE, 2026-09-21 (R5). Today's `docs/backlog-2026-06.md` is D-B's"
