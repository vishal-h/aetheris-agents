...
| **End-to-end fork** (human-verified exception) | **Headless portion verified:** created a stub forkable source in the dev store, ran the real subprocess `mix aetheris --json fork <traj> --step 0` → stdout `{"status":"done","run_id":"fork-80cf8333fb4edc14"}`; `parse_run_id` emulation extracted `fork-80cf8333fb4edc14`; forked meta = `fork_from: t3-fork-src-452`, `fork_step: 0`, `mode: "record"`, `seed: 7` (carried). **Human-verified 2026-07-19:** `fork_run` invoked from running Rig via devtools console (temporary `withGlobalTauri` flip, reverted); returned `fork-e27268b7bdbe14c8`; trajectory meta confirmed `fork_from`/`fork_step`/`mode: "record"`/seed carried (and `user_prompt: ""` — the F1 fix visible in the artifact); bad-step invoke rejected with `:step_not_found` stderr (F3 path through IPC). No verification debt carries to t4. |
