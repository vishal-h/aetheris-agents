...
**Status:** In progress (T0, T1, T2 complete; T3 in progress)
...
| Running the orchestrator against `sample_payroll.csv` produces `output/BTL_999/*.html` | 🔲 T3 |
| `output/BTL_999/merged.pdf` generated | 🔲 T3 |
| `mix aetheris tree show <run_id>` shows orchestrator + sub-agent tree | 🔲 T3 |
| `./scripts/sprint.sh payslip` passes | 🔲 T3 |
| All aetheris CI checks pass | 🔲 T3 |
| `README.md` and `runbook.md` complete | 🔲 T3 |
...
T0 (aetheris) — spawn_agent sandbox inheritance          ✅ ad3675f
  └─ T1 (aetheris-agents) — payslip_compute.py          ✅ 2e8b0e1
     ...
     T2-Part-B (aetheris-agents) — merge_payslips.py    ✅ f7f31a3
       └─ T3 (aetheris-agents + aetheris) — agent + docs + sprint  🔲
```
...
| T3 | `aetheris-agents` + `aetheris` | `t3-payslip-agent` | 🔲 |
...
**Branch:** `t3-payslip-agent`
**Depends on:** T0, T1, T2 all merged
...
**`payslip/milestone.md`:** this document, updated to reflect T3 completion.
...
**Acceptance criteria:**
- Orchestrator runs end-to-end against `sample_payroll.csv`
- `output/BTL_999/*.html` files written with correct content
- `output/BTL_999/merged.pdf` generated
- `mix aetheris tree show <run_id>` shows full agent tree
- `./scripts/sprint.sh payslip` passes
- README.md and runbook.md complete
- All aetheris CI checks pass
...
aetheris-agents/
  ...
  payslip/
    ...
    docs/
      t0-implementation-notes.md
      t1-implementation-notes.md    ← T1 (to write)
      t2-implementation-notes.md    ← T2 (to write)
