...
def test_btl_999_exits_zero_and_produces_html_files(tmp_path):
    result = run_script("BTL_999", tmp_path)
    assert result.returncode == 0, result.stderr
    emp_dir = tmp_path / "BTL_999"
    assert (emp_dir / "2026-04.html").exists()
    assert (emp_dir / "2026-03.html").exists()
...
def test_btl_999_apr_2026_html_content(tmp_path):
    result = run_script("BTL_999", tmp_path)
    assert result.returncode == 0, result.stderr
    html = (tmp_path / "BTL_999" / "2026-04.html").read_text()
...
@pytest.mark.integration
def test_btl_999_mar_2026_contains_bonus_row(tmp_path):
    result = run_script("BTL_999", tmp_path)
    assert result.returncode == 0, result.stderr
    html = (tmp_path / "BTL_999" / "2026-03.html").read_text()
    assert "Bonus" in html


@pytest.mark.integration
def test_btl_998_stipend_html_content(tmp_path):
    result = run_script("BTL_998", tmp_path)
    assert result.returncode == 0, result.stderr
    html = (tmp_path / "BTL_998" / "2026-04.html").read_text()
...
@pytest.mark.integration
def test_btl_997_consultant_html_content(tmp_path):
    result = run_script("BTL_997", tmp_path)
    assert result.returncode == 0, result.stderr
    html = (tmp_path / "BTL_997" / "2026-04.html").read_text()
